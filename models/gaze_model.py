"""
Архитектура нейронной сети для отслеживания взгляда
"""
import torch
...
class GazeTrackingModel(nn.Module):
    """Модель для предсказания координат взгляда"""
    ...
    def forward(self, x):
        ...
        gaze_prediction = self.regressor(features)
...
class MultiModalGazeModel(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            self.eye_net = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)
        else:
            self.eye_net = models.resnet18(weights=None)
        self.eye_net.fc = nn.Identity()  # Удаляем последний слой
        ...
        self.fusion = nn.Sequential(
            nn.Linear(512 + 128, 256),  # 512 от eye_net + 128 от head_pose_net
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(256, 128),
            nn.ReLU(),
